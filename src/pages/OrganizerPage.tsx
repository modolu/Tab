...
export default function OrganizerPage() {
  ...
  if (tab === undefined) return <div className="page state-page"><div className="loading-pulse" aria-hidden="true" /><p className="muted-label">Loading your share view…</p></div>
  ...
  async function copyShareLink() {
    try { await navigator.clipboard.writeText(shareUrl); setCopied(true); window.setTimeout(() => setCopied(false), 1800) } catch { setCopied(false) }
  }
  ...
  return (
    <div className="page page-tab organizer-page">
      ...
      <section className="success-card"><span className="success-icon" aria-hidden="true">✓</span><div><h3>{tab.status === 'settled' ? 'Tab settled' : 'Your Tab is ready'}</h3><p>{tab.status === 'settled' ? 'Every contribution has been verified.' : 'Share the link with everyone contributing.'}</p></div></section>
